...
@kanjeng.alfareza_cmd(
    ...
    info={
        "header": "To get guide for kanjeng userbot.",
...
async def _(event):
    "To get guide for kanjeng userbot."
...
async def _(event):
    ...
    if not input_str:
        ...
        outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help -c <command name>`"
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of PetercordBot are :"
    )
...
async def _(event):
    ...
    if found:
        ...
        out = f"I can't find any such command `{cmd}` in PetercordBot"
