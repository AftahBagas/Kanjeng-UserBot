...
async def fakegs(search, result):
    imgurl = "https://imgur.com/gallery/ieSTXbM"
    ...
    font1 = ImageFont.truetype(
        "pyPetercord/helpers/styles/ProductSans-BoldItalic.ttf", 20
    )
    font2 = ImageFont.truetype("pyPetercord/helpers/styles/ProductSans-Light.ttf", 23)
...
async def trumptweet(text):
    ...
    ilham = r.get("message")
    petercordurl = url(ilham)
    if not petercordurl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(ilham).content)
